...
function tf_render_table(array $poll): void
{
    ...
    <div class="table-wrapper">
        <table class="responses">
            <caption class="sr-only">Übersicht der bisherigen Antworten</caption>
            <thead>
                <tr>
                    <th scope="col">Teilnehmende</th>
                    <?php foreach ($poll['slots'] as $slot): ?>
                        <th scope="col"><?php echo tf_h($slot['slot_label']); ?></th>
                    <?php endforeach; ?>
                </tr>
            </thead>
            <tbody>
...
<main class="container" id="main-content">
    ...
    <section class="card">
        <h2>Teilnehmen</h2>
        <form method="post" aria-describedby="vote-help">
            <div class="grid">
                <label>Name*<br><input type="text" name="name" required value="<?php echo tf_h($_POST['name'] ?? ''); ?>"></label>
                <label>Kommentar (optional)<br><input type="text" name="comment" value="<?php echo tf_h($_POST['comment'] ?? ''); ?>"></label>
            </div>
            <div class="table-wrapper">
                <table class="vote-table">
                    <caption class="sr-only" id="vote-help">Für jeden Termin einen Status auswählen.</caption>
                    <thead>
                        <tr>
